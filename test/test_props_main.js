...
describe("Test property insert - main graph", () => {
        ...
        /**
         * It should not be possible to add a property to some FoI without specifying which property
         */
        it('Try to add a property by only specifying a FoI and not a property (CONSTRUCT)', async () => {
        ...
        /**
         * It should not be possible to add a property to some FoI without specifying what FoI
         */
        it('Try to add a property by only specifying the property and neither a property or a path (CONSTRUCT)', async () => {
        ...
        /**
         * It should not be possible to add a property to some FoI without specifying a value
         */
        it('Try to add a property without giving it a value (CONSTRUCT)', async () => {
        ...
        /**
         * It should not be possible to add a property to some FoI which doesn't exist
         */
        it('Try to add a property to a FoI that does not exist (CONSTRUCT)', async () => {
            
            var input = {
                foiURI: 'ex:someUndefinedFoI',
                property: 'props:designAmbientTemperature',
                value: '"70 Cel"^^cdt:temperature'
            };
    ...
    describe("Test property update - main graph", () => {
        ...
        it('Check that property now has 2 states', async () => {
            ...
            WHERE {
                ex:FoI props:designAmbientTemperature/seas:evaluation ?state .
            }`;
...
describe("Property reliability - main graph", () => {
    ...
    /**
     * It should not be possible to explicitly define reliability opm:Derived as this should be inferred
     */
    it("Try setting reliability to 'derived' (CONSTRUCT)", async () => {
        ...
        expect(q).to.be.an('error');

    });

    /**
     * Reliability should be restricted to a set of predefined classes
     */
    it("Try setting reliability to 'somethingRandom' (CONSTRUCT)", async () => {
        
        var input = {
            propertyURI: propURI,
            reliability: 'somethingRandom'
        };
    ...
    /**
     * Setting reliability to 'confirmed' is only possible when specifying a URI referring to to an agent
     */
    it("Try setting reliability to 'confirmed' without specifying a user (CONSTRUCT)", async () => {
    ...
    /**
     * Get latest state of all properties in the store
     */
    it('Get latest state of all properties', async () => {

        var q = opmProp.getAllProps('select');

        const res = await query.execute(conn, dbName, q);
        
        expect(res).to.have.property('status').that.is.equals(200);     // Should return status 200
        expect(res.body.results.bindings).to.be.an('array').to.have.length(2);   // Should return a body with length 2 (designAmbientTemperature for FoI and FoI2)
