...
var OPMProp = (function (_super) {
    ...
    OPMProp.prototype.postFoIProp = function (input) {
    ...
    //Update FoI property
    OPMProp.prototype.putFoIProp = function (input) {
        //Retrieve and process variables
        var property = input.inferredProperty;
        var value = input.value.value;
        var datatype = input.value.datatype;
    ...
    OPMProp.prototype.listOutdated = function (input) {
        var foiURI = input.foiURI;
        var queryType = input.queryType ? input.queryType : this.queryType;
        var q = '';
        //Define prefixes
    ...
    //Delete property
    OPMProp.prototype.deleteProp = function (input) {
        var args = input;
     ...
     * OTHER
     */
    OPMProp.prototype.listSubscribers = function (input) {
        var propertyURI = input.propertyURI;
        var queryType = input.queryType ? input.queryType : this.queryType;
        var q = '';
        q += 'PREFIX  opm: <https://w3id.org/opm#>\n';
    ...
    OPMProp.prototype.cleanPath = function (path) {
    ...
    OPMProp.prototype.reliabilityClass = function (reliability) {
        var options = _.filter(this.reliabilityOptions, function (obj) { return (obj.key != 'derived'); }); //Derived can not be set as it is only inferred for derived properties
